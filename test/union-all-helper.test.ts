...
describe("unionAllPairwise ownership", () => {
    ...
    it("releases all helper-owned intermediates when a later fuse fails", () => {
        const fake = new FakeKernel();
        ...
        // The first successful pair produced handle 100. The second pair
        // failed before producing a new handle, so 100 is the only helper-owned
        // shape that must be reclaimed. Inputs remain caller-owned.
    ...
    it("copies a single input so the returned handle has caller-owned result semantics", () => {
        const fake = new FakeKernel();
    ...
    it("rejects an empty input without touching the kernel", () => {
        const fake = new FakeKernel();
